...
OrgData =[X Y];
Accuracy = zeros((relevantSize/2),K);
NumberOfSelectedF = zeros((relevantSize/2),K);
time1 = zeros((relevantSize/2),K);
...
Accuracy(GroupSize,fold) = ClassifyC45_Test([data(:,selectedF) data(:,end)],[OrgData(teIdx,relevantFeatures(selectedF)) , Y(teIdx)]);
...
save(['results_C45_GeneralSFS_fold_mySFS_minfold\' cell2mat(DataName)],'CVO','meanAcc','Accuracy','NumberOfSelectedF','selectedF','newDATA','time1');
end
