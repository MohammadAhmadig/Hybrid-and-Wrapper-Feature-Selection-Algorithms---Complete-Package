function [featIndex, meanACCval, meanACCtr, numJ] = IGIS(inp, ground, CVO, i, opt, K)
% Songyot Nakariyakul
...
Pc = zeros(1,numFtr); % PC is the mutual information for numFtr features
for ii = 1:numFtr
    Pc(ii) = mi(input(:,ii),groundtruth); % mutual information
end
...
while isempty(find(restFtrs, 1)) == 0
    ...
    for ss = 1:lengthidx
       ...
       % if (cohen_d(ACtr, maxACCtr) >= 0.4)    % Use Cohen's d to test significant improvement for training set accuracy
        if (ttest( maxACCtr,ACtr ,'Alpha',0.1,'Tail','left')==1) 
            ...
            if (ttest( ACval,maxACCval ,'Alpha',0.1,'Tail','left')==1)   
                q = 1;
                break;
            else
